...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'model',
      text: `Hi ${user?.displayName?.split(' ')[0] || 'there'}! I'm your WiseBite Health AI. Ask me anything about nutrition, get a meal plan, or upload a food photo to log it instantly.`,
    },
  ]);
    ...
    <div style={{ alignSelf: isUser ? 'flex-end' : 'flex-start', maxWidth: '88%' }}>
      {!isUser && (
        <div className="text-xs flex items-center gap-1 mb-1" style={{ color: 'var(--text-secondary)' }}>
          <Sparkles size={11} color="var(--primary-dark)" /> WiseBite AI
        </div>
      )}
      <div style={{
